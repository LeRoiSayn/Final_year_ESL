...
import { BookOpenIcon, ChartBarIcon, ClockIcon, CurrencyDollarIcon } from '@heroicons/react/24/outline'
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Student Dashboard</h1>
            ...
            {stats?.courses?.slice(0, 5).map((enrollment) => (
              <div key={enrollment.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-dark-300">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500 to-teal-500 flex items-center justify-center"><BookOpenIcon className="w-5 h-5 text-white" /></div>
                  <div><p className="font-medium">{enrollment.class?.course?.name}</p><p className="text-sm text-gray-500">{enrollment.class?.course?.code}</p></div>
                </div>
                <span className="badge badge-info">{enrollment.class?.course?.credits} cr</span>
              </div>
            ))}
            {(!stats?.courses || stats.courses.length === 0) && <p className="text-gray-500 text-center py-4">No courses enrolled</p>}
          </div>
          <Link to="/student/courses" className="block mt-4 text-center text-primary-600 hover:text-primary-700 font-medium">View all courses →</Link>
        </motion.div>
